...
#     """
#     with torch.no_grad():
#         tensor.normal_(mean=mean, std=std)  # Generate normal distribution
#         while True:
#             mask = (tensor < mean - 2 * std) | (tensor > mean + 2 * std)  # Mask for values outside truncation range
#             if not mask.any():
#                 break
#             tensor[mask] = torch.normal(mean, std, size=mask.sum().size())  # Resample values outside range
#     return tensor
...
class Decoder(nn.Module):
    ...
    def forward(self, soft_input, labels, code):
        loss = 0.0
        cv = torch.zeros(num_edges, batch_size, dtype=torch.float32)
        vc = soft_input.clone()
    ...
    def compute_vc(self, cv, soft_input, code):
        ...
        for i in range(code.n):  # for each variable node
            for j in range(code.var_degrees[i]):
                ...
                if extrinsic_edges:
                    ...
                    temp = torch.zeros(cv.size(1), dtype=cv.dtype, device=cv.device)  # 无外部边则置 0
        ...
        # 按照边的顺序重新排列
        new_order = torch.zeros(num_edges, dtype=torch.int64, device=vc.device)
        new_order[edge_order] = torch.arange(0, num_edges, dtype=torch.int64, device=vc.device)
        vc = vc[new_order]
    ...
    def compute_cv(self, vc, iteration, code):
        ...
        if SUM_PRODUCT:
            # 汇总 SUM_PRODUCT 结果
            cv = torch.stack(cv_list)
        elif MIN_SUM:
            # 汇总 MIN_SUM 结果
            prods = torch.stack(prod_list)
            mins = torch.stack(min_list)
            if self.decoder_type == "RNOMS":
                mins = torch.relu(mins - self.B_cv)
            elif self.decoder_type == "FNOMS":
                offset = torch.nn.functional.softplus(self.B_cv[iteration])
                offset = offset.unsqueeze(1).repeat(1, batch_size)
                mins = torch.relu(mins - offset)
            cv = prods * mins

        # 重新排列 cv 的顺序
        new_order = torch.zeros(num_edges, dtype=torch.int64, device=cv.device)
        new_order[edge_order] = torch.arange(num_edges, dtype=torch.int64, device=cv.device)
        cv = cv[new_order]

        # 应用权重调整
        if self.decoder_type in ["RNSPA", "RNNMS"]:
            cv = cv * self.W_cv.unsqueeze(1)
        elif self.decoder_type in ["FNSPA", "FNNMS"]:
            cv = cv * self.W_cv[iteration].unsqueeze(1)
...
decoder = Decoder(decoder_type="FNOMS", num_edges=num_edges, num_iterations=num_iterations, relaxed=True)
optimizer = optim.Adam(decoder.parameters(), lr=learning_rate)

model_path = os.path.join(save_path, "decoder_step_2000_lst10_200.pth")  # Replace with the correct saved model name
...
for SNR in SNRs:
    ...
    while (FE < min_frame_errors or frame_count < min_frame) and frame_count < max_frames:
        ...
        # Run the decoder
        _, soft_output = decoder(batch_data, codewords, code)
